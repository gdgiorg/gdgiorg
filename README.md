[README.md](https://github.com/user-attachments/files/32346243/README.md)
...
**A post can carry its own photo gallery and a YouTube video**, beyond the single `photo:` field every post has for its card/hero image. Add `gallery: [{ src: 'assets/photos/<file>.jpg', alt: '…' }, …]` to a `posts[…]` entry in `generator/data.mjs` for a grid of extra photos at the bottom of that post (see the `stakeholders-meeting-national-summit` post for a 14-photo example), and/or `youtubeId: '<the 11-character ID from the video URL>'` to embed a responsive, privacy-enhanced (`youtube-nocookie.com`) video above it. Both are optional and independent of each other and of `photo:`.
